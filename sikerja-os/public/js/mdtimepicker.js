...
+(function ($) {
  ...
  var Time = function (hour, minute) {
    ...
    this.invert = function () {
      if (this.getT() === "Ù‚.Ø¸") this.setHour(this.getHour() + 12);
  ...
  var MDTimePicker = function (input, config) {
    ...
    this.timepicker = {
      ...
      timeHolder: {
        ...
        am_pm: $('<span class="mdtp__ampm">Ù‚.Ø¸</span>'),
      },
      clockHolder: {
        wrapper: $('<section class="mdtp__clock_holder"></section>'),
        am: $('<span class="mdtp__am">Ù‚.Ø¸</span>'),
        pm: $('<span class="mdtp__pm">Ø¨.Ø¸</span>'),
        clock: {
        ...
        buttonsHolder: {
          ...
          btnOk: $(
            `<span class="mdtp__button ok">${
              that.config.okLabel || "ØªØ§Ø¦ÛŒØ¯"
            }</span>`
          ),
          btnCancel: $(
            `<span class="mdtp__button cancel">${
              that.config.cancelLabel || "Ø§Ù†ØµØ±Ø§Ù"
            }</span>`
          ),
        },
      },
    ...
    picker.clockHolder.am.click(function () {
      if (that.selected.getT() !== "Ù‚.Ø¸") that.setT("am");
    });
    picker.clockHolder.pm.click(function () {
      if (that.selected.getT() !== "Ø¨.Ø¸") that.setT("pm");
    });
  ...
  MDTimePicker.prototype = {
      ...
      for (var i = 0; i < 12; i++) {
        ...
        hour.find("span").click(function () {
          var _data = parseInt($(this).parent().data("hour")),
            ...
            _value =
              (_data +
                ((_selectedT === "Ø¨.Ø¸" && _data < 12) ||
                (_selectedT === "Ù‚.Ø¸" && _data === 12)
                  ? 12
                  : 0)) %
              24;
    ...
    setT: function (value) {
      ...
      this.timepicker.clockHolder.am[t === "Ù‚.Ø¸" ? "addClass" : "removeClass"](
        "active"
      );
      this.timepicker.clockHolder.pm[t === "Ø¨.Ø¸" ? "addClass" : "removeClass"](
        "active"
      );
    },
